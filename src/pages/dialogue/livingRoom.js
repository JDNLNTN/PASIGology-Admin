...
export default function IntroductionDialogue() {
  ...
  const { dialogues, loading, error, addDialogue, updateDialogue, deleteDialogue } = useDialogueManager('livingroomscript');
 const { role: currentUserRole } = useAuth();
  ...
  const [newSpeakerText, setNewSpeakerText] = useState(''); // Default to Aji
  ...
  const speakers = ['m', 'p']; // m = mom Character, p = player Character
  ...
  return (
    <Container maxWidth="lg">
      <Typography variant="h4" gutterBottom>
        Living Room Dialogue Management
      </Typography>
      {!currentUserRole && (
      ...
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Typography variant="body1">Note: M = Mom Character, P = Player Character</Typography>
              ...
              <SortableContext items={dragDialogues.map(d => d.id.toString())} strategy={verticalListSortingStrategy}>
                {dragDialogues.map((dialogue, index) => (
                  <SortableDialogueRow
                    ...
                    handleUp={() => {
                      if (index > 0) {
                        const upOrder = Array.from(dragDialogues);
                        const upTemp = upOrder[index - 1];
                        upOrder[index - 1] = upOrder[index];
                        upOrder[index] = upTemp;
                        setDragDialogues(upOrder.map((item, idx) => ({ ...item, sequence: idx + 1 })));
                      }
                    }}
                    handleDown={() => {
                      if (index < dragDialogues.length - 1) {
                        const downOrder = Array.from(dragDialogues);
                        const downTemp = downOrder[index + 1];
                        downOrder[index + 1] = downOrder[index];
                        downOrder[index] = downTemp;
                        setDragDialogues(downOrder.map((item, idx) => ({ ...item, sequence: idx + 1 })));
                      }
                    }}
                    disableUp={index === 0}
