...
export default function KuyaReneDialogue() {
  const { dialogues, loading, error, addDialogue, updateDialogue, deleteDialogue } = useDialogueManager('renescript');
  ...
  const [newSpeakerText, setNewSpeakerText] = useState('r'); // Default to Rene
  const [newSequenceText, setNewSequenceText] = useState('');
  const [componentError, setComponentError] = useState(null);

  const speakers = ['r', 'p']; // r = Rene Character, p = Player Character
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewDialogueText('');
      setNewSpeakerText('r'); // Reset to default Rene
      setNewSequenceText('');
    ...
    <DialogueLayout>
      <Container maxWidth="lg">
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Typography variant="body1">Note: r = Rene Character, p = Player Character</Typography>
