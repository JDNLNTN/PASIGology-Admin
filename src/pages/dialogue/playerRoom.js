...
export default function PlayerRoomDialogue() {
  const { dialogues, loading, error, addDialogue, updateDialogue, deleteDialogue } = useDialogueManager('playerroomscript');
  ...
  const [newSpeakerText, setNewSpeakerText] = useState('p'); // Default to Player
  const [newSequenceText, setNewSequenceText] = useState('');
  const [componentError, setComponentError] = useState(null);

  const speakers = ['p', 'm']; // p = Player Character, m = Mother Character
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(`Error updating dialogue: ${err.message}`);
    }
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewDialogueText('');
      setNewSpeakerText('p'); // Reset to default Player
      setNewSequenceText('');
    ...
    <DialogueLayout>
      <Container maxWidth="lg">
        <Typography variant="h4" gutterBottom>
          Player Room Dialogue Management
        </Typography>
        ...
        <Paper sx={{ p: 2, mb: 2 }}>
          ...
          <Typography variant="body1">Note: p = Player Character, m = Mother Character</Typography>
