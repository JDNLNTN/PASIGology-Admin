...
export default function ScriptCustomerDialogue() {
  const { dialogues = [], loading, error, addDialogue, updateDialogue, deleteDialogue } = useDialogueManager('script_customer');
  ...
  const [newSpeakerText, setNewSpeakerText] = useState('c'); // Default to customer
  const [newSequenceText, setNewSequenceText] = useState('');
  const [componentError, setComponentError] = useState(null);

  const speakers = ['c', 'p']; // c = customer, p = player
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewDialogueText('');
      setNewSpeakerText('c'); // Reset to default customer
      setNewSequenceText('');
  ...
  return (
    <DialogueLayout>
      <Box p={3}>
        <Typography variant="h4" gutterBottom>Customer Dialogue Management</Typography>
        <Paper sx={{ p: 2, mb: 2 }}>
          <Typography variant="h6">Add New Dialogue</Typography>
          <Typography variant="body1">Note: c = Customer Character, p = Player Character</Typography>
