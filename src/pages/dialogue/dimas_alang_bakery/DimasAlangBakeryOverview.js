import React from 'react';
import { useNavigate } from 'react-router-dom';
import { Card, Table, Button } from 'react-bootstrap';
import DialogueLayout from '../DialogueLayout';

function DimasAlangBakeryOverview() {
  const navigate = useNavigate();

const dialoguePages = [
  { name: 'Ate Isabella Script', path: '/dialogue/dimas_alang_bakery/script_ate_isabella' },
  { name: 'Customer Script', path: '/dialogue/dimas_alang_bakery/script_customer' },
];

  const handleManage = (path) => {
    navigate(path);
  };

  return (
    <DialogueLayout>
      <div className="dimas-alang-bakery-overview">
        <h2>Dimas-Alang Bakery Scenario</h2>
        <Card>
          <Card.Body>
            <Table striped bordered hover>
              <thead>
                <tr>
                  <th>Scenario</th>
                  <th>Action</th>
                </tr>
              </thead>
              <tbody>
                {dialoguePages.map((page, index) => (
                  <tr key={index}>
                    <td>{page.name}</td>
                    <td>
                      <Button
                        variant="primary"
                        size="sm"
                        onClick={() => handleManage(page.path)}
                      >
                        Manage
                      </Button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </Table>
          </Card.Body>
        </Card>
      </div>
    </DialogueLayout>
  );
}

export default DimasAlangBakeryOverview;