...
export default function GalleryInfoManager({
  ...
  const onDragStart = (e, id) => {
    if (isReadonly || !sequenceField) return;
    setDragId(id);
  ...
  const handleApprove = async (id) => {
    if (isReadonly || !approveField) return;
    setSaving(true);
    const updatePayload = {};
  ...
  const handleImplementedToggle = async (id) => {
    if (isReadonly || !implementedField) return;
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {orderedRows.map((row) => (
                <tr
                  key={row[idField]}
                  draggable={!isReadonly && !!sequenceField}
                  onDragStart={(e) => !isReadonly && !!sequenceField && onDragStart(e, row[idField])}
                  onDragOver={(e) => !isReadonly && !!sequenceField && onDragOver(e)}
                  onDrop={(e) => !isReadonly && !!sequenceField && onDrop(e, row[idField])}
                  style={trStyle}
                >
                  <td style={tdStyle}>
                    <span style={{ cursor: isReadonly || !sequenceField ? 'default' : 'grab' }}>⋮⋮</span> {sequenceField ? (row[sequenceField] ?? '-') : '-'}
                  </td>
                  ...
                  <td style={tdStyle}>
                    <button onClick={() => handleEdit(row[idField])} disabled={saving || isReadonly}>Edit</button>{' '}
                    {!approveField || row[approveField] ? (
                      <button onClick={() => handleImplementedToggle(row[idField])} disabled={saving || isReadonly || !implementedField}>
                        {implementedField && row[implementedField] ? 'Mark Not Implemented' : 'Mark Implemented'}
                      </button>
                    ) : (
                      <button onClick={() => handleApprove(row[idField])} disabled={saving || isReadonly}>Approve</button>
                    )}{' '}
                    <button onClick={() => handleDelete(row[idField])} disabled={saving || isReadonly} style={{ color: 'red' }}>
                      Delete
                    </button>
                  </td>
                </tr>
